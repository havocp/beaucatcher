...
private[cdriver] final class ChannelDriverSyncDatabase(override protected val database: ChannelDriverDatabase) extends SyncDatabase {
...
private[cdriver] final class ChannelDriverAsyncDatabase(override protected val database: ChannelDriverDatabase) extends AsyncDatabase {
    override def command(cmd: BObject, options: CommandOptions): Future[CommandResult] = {
        database.command(cmd, options)
    }
}

private[cdriver] final class ChannelDriverDatabase(override val context: ChannelDriverContext, override val name: String) extends Database {
